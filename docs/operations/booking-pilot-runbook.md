...
`calendar` оставляет внутренний календарь и закрывает public/manage routes. `off` закрывает обе поверхности, сохраняя bookings, visits, audit и financial snapshots. Повышать до `public` может только оператор; владелец может понижать уровень сам.
