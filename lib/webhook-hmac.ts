 ...
 * in constant time for the same reason the operator token in
 * `app/api/v1/ops/notifications/route.ts` is — a comparison that returns on
 * the first mismatched byte leaks the secret one byte at a time.
 */
